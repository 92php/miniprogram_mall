...
class Order
{
    ...
    {
        $status = [
            'pass' => true, //订单库存量检测是否通过
            'orderPrice' => 0,
            'totalCount' => 0,
            'pStatusArray' => []    //订单商品的所有信息
        ];

        foreach ($this->oProducts as $item) {
            $productInfo = $this->getProductStatus($item['product_id'], $item['count'], $this->products);
